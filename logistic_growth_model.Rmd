...
We can account of a limited growth by applying a logistic growth model. Here the growth is being slowest at the onset and at the end of the epidemic. It is considered in the model, that the number of infections approaches a maximum value (Nmax) and that the growth rate $mu$ decreases as N increases .

mu0<-k*log(Wmax/W0)
W0*exp(mu0/k*(1-exp(-k*times))

* $N=N_0*e^{ln(N_{max}/N_0)*(1-e^{-kt})}$ 


### Nonlinear Least Square (NLS) FIT

A NLS model is used to determine the parameters $N_{max}$ and $k$, the maximum number of infections and growth rate. 
...
require(gridExtra)
p1<-ggplot()+ 
  geom_point(data=fitGER, aes(x = Date, y = cumulative_cases, color="Cumulative Cases"), pch=21, cex=2)+
...
```{r echo=FALSE}
ggplot()+ 
  geom_col(data=fitGER, aes(x = Date, y = incident_cases, color="Incident Cases"), alpha=0.5)+
...
```{r echo=FALSE}
fitCH<-filter(confirmedCases_JHU, country_region == "Switzerland", Date>="2020-03-01")%>%
  mutate(time=Date-ymd("2020-03-01"))%>%
  mutate(incident_cases = c(0, diff(cumulative_cases)))
#initial values
N0<-27
...
require(gridExtra)
p1<-ggplot()+ 
  geom_point(data=fitCH, aes(x = Date, y = cumulative_cases, color="Cumulative Cases"), pch=21, cex=2)+
...
```{r}
ggplot()+ 
  geom_col(data=fitCH, aes(x = Date, y = incident_cases, color="Incident Cases"), alpha=0.5)+
